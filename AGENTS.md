...
- **Async subagent widget (`fix/async-subagent-widget`):** above-editor widgets (todos vs async) are separated by a blank row and a dim rule. A running async child uses the same description-first compact row as foreground (`modelSelection` badge, `⎿ tools · tokens · time`). The running glyph advances on an 80ms paint timer. Tests: subagent-compact-row.
...
- 2026-09-09: async subagent widgets match compact foreground rows, animate the running glyph, and sit below todos with a divider.
