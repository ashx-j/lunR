...
- **Injected context chat (`feat/injected-context-chat`):** a new session with no user/assistant messages prints an `injected context` chat card listing loaded AGENTS.md/CLAUDE.md files. The model still gets those files through the system prompt. Tests: injected-context.
...
- 2026-09-09: starting a chat lists injected AGENTS.md files in the transcript as `injected context`.
