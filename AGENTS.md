...
- **#50 reverted:** `fix/cold-start-readiness` (lazy first-request web/LSP/MCP/subagent engines and the Node split bundle) landed on master and was reverted. First paint from 0.2.14 stays.
...
- **Async subagent widget (`fix/async-subagent-widget`):** todos and the async widget are separated by a blank row and a dim rule. A running async child uses the same description-first compact row (`modelSelection` badge, hang line of tools · tokens · time). The glyph advances on an 80ms paint timer. Tests: compact-row.
...
- **Injected context chat (`feat/injected-context-chat`):** a new session with no user or assistant messages prints an `injected context` card listing loaded AGENTS.md/CLAUDE.md files. The model still receives those files through the system prompt. Tests: injected-context.
...
- Cold-start readiness (`fix/cold-start-readiness`, #50) was merged then reverted. The branch still exists.
...
- 2026-09-09: empty new chats list injected AGENTS.md/CLAUDE.md files in an injected context card.
- 2026-09-12: revert #50 so Node split bundles and on-demand web/LSP/MCP/subagent engines are not on master.
