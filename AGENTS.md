...
Last updated: 2026-08-29 (Codex Fast and usage cleanup). **`origin/master` = `10c801b`**. Public npm is `@ashx-j/lunr@0.2.11` (tag `v0.2.11`). **NEVER MERGE `archive/extension-absorption-DO-NOT-MERGE`.** Untracked locals: `prompts/`, `DESIGN.md`, `LUNR_SYSTEM_INJECTION.md`, `lunR-checklist.md`, `.pi-subagents/`.

- **Codex Fast + usage cleanup (`feat/codex-fast-usage-cleanup`):** `/fast [on|off|status]` persists `service_tier: "fast"` for `openai-codex` subscriptions only and marks the model chip. `/usage` fetches every stored-credential adapter plan plus the current env-only provider in parallel; Codex includes additional rate limits. Behavior remains user-managed but has no agent tools, and file tools cannot change `behavior.md`. The `/research` pipeline is removed; web tools and generic research subagents remain.
...
- **Behavior presets (`feat/behavior-presets`):** `/settings` Behavior preset = default (empty, fill later) / humanizer / concise / custom. Built-ins replace `~/.lunr/agent/behavior.md`. Custom keeps the user-managed file; no in-app editor, agent editing tools, or `fs.watch`. Fingerprint (header stripped) on `/settings` open and each `before_agent_start`. Drift off a template flips to custom. Overwrite confirm when replacing a custom file with a built-in. Built-ins skip `memoryCharCap`; custom is capped. Tests: `behavior-preset.test.ts`.
...
- Behavior preset custom does not open an editor. Only the user edits `~/.lunr/agent/behavior.md`; agent file tools are blocked from that path. Do not add behavior tools or `fs.watch`.
...
- 2026-08-29: Behavior is user-managed and agent-read-only; remove the tool surface and block direct file-tool writes.
