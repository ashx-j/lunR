...
async function probeTui(artifactRoot, cli, anchor, standalone) {
	...
	return { firstPaint: result.tuiFirstPaint, settings: result.settingsDialogSeen, artifactNativeSpawn: result.artifactNativeSpawn, reattachColumns: result.repaintColumns };
}
...
async function proveStandalone() {
	...
	const wrapperPath = assertCandidateFromArtifact(artifact, artifact);
	report.standalone = { result: "blocked", compiledCli: true, wrapperPath, candidateVersion, nativeBundle: selected, artifactNativeTui: "not-run" };
	const tui = await probeTui(artifact, cli, artifact, true);
	report.standalone = { result: "passed", compiledCli: true, wrapperPath, candidateVersion, nativeBundle: selected, tui };
}
