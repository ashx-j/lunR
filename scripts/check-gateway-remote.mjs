...
const server = http.createServer(async (request, response) => {
	try {
		...
		if (requests === 0) {
			...
			fs.writeFileSync(path.join(root, "LUNR_SYSTEM_INJECTION.md"), body.messages.filter((message) => message.role === "system" || message.role === "developer").map((message) => message.content).join("\n\n"));
		}
		requests++;
...
fs.writeFileSync(path.join(agentDir, "settings.json"), JSON.stringify({ defaultProvider: "fixture", defaultModel: "local", defaultThinkingLevel: "off", defaultPermissionMode: "manual", memoryEnabled: false, retry: { enabled: false }, compaction: { enabled: false } }));
...
const [{ AgentBridge }, { createRouter }, { loadGatewayConfig, saveGatewayConfig }, { createPairingStore }, presentation, { SessionManager }, handoff, { buildSessionKey }, approval] = await Promise.all([
	load("gateway/agent-bridge.js"), load("gateway/router.js"), load("gateway/config.js"), load("gateway/pairing.js"), load("gateway/presenter.js"), load("core/session-manager.js"), load("core/session-handoff.js"), load("gateway/session-keys.js"), load("gateway/approval.js"),
]);
...
const adapter = {
	...
	send: async (_chat, text) => { delivered.push(text); return { success: true, messageId: String(++nextId) }; },
...
desktop.flush();
desktop.setPermissionMode("manual");
...
try {
	...
	assert(approved, "The real write tool bypassed the phone approval");
	assert(delivered.some((text) => text.includes("gateway-local-ok")), delivered.join("\n"));
	...
	try {
		...
		assert.equal(reclaimed.getPermissionMode(), "manual");
	...
	assert.equal(handoff.listHandoffCandidates().length, 1, "Continuation consumed the manual mark");
	console.log("PASS: compiled gateway, desktop continuation, original project, local model turn, phone write approval, file download and desktop reclaim.");
} catch (error) {
	process.exitCode = 1;
	console.error(error);
} finally {
	...
	fs.mkdirSync(path.join(root, ".artifacts"), { recursive: true });
	fs.writeFileSync(path.join(root, ".artifacts", "gateway-local-output.json"), JSON.stringify({ requests, approved, delivered, files, profile: temporary }, null, 2));
	console.log(`Isolated profile: ${temporary}`);
}
