...
registerHooks({ load(url, context, nextLoad) {
 if (/\\/coding-agent\\/dist\\/(?:node-runtime\\/)?main\\.js$/.test(url)) return { format: "module", shortCircuit: true, source: ${JSON.stringify(fixture)} };
...
async function checkRequest(toolUrl) {
	...
	const blocked = optionalModules.map((name) => pathToFileURL(join(root, "packages/coding-agent/dist/builtin-extensions", name)).href);
	if (existsSync(join(root, "packages/coding-agent/dist/node-runtime/cli-runtime.js"))) {
		...
		for (const [output, details] of Object.entries(metadata.outputs)) {
			if (Object.keys(details.inputs).some((input) => optionalModules.some((name) => input.endsWith(`/builtin-extensions/${name}`)))) {
				blocked.push(pathToFileURL(resolve(root, output)).href);
			}
		}
	}
	const preload = `import { registerHooks } from "node:module";
...
registerHooks({load(url, context, nextLoad) {
 ...
 return nextLoad(url,context);
	...
	const child = spawn(process.execPath, [...(toolUrl ? [] : ["--import", `data:text/javascript,${encodeURIComponent(preload)}`]), cli, "--no-session", "--no-approve"], {
		cwd: workspace,
		env: { ...process.env, HOME: home, USERPROFILE: home, APPDATA: home, LOCALAPPDATA: home, TMP: temp, TEMP: temp, TMPDIR: temp,
			PI_CODING_AGENT_DIR: agentDir, PI_STARTUP_BENCHMARK: "1", PI_TIMING: "1", PI_OFFLINE: "1", PI_SKIP_VERSION_CHECK: "1",
			...(toolUrl ? { PI_STARTUP_BENCHMARK_TOOL_URL: toolUrl } : {}),
		},
	...
	try {
		...
		for (const name of ["read", "bash", "edit", "write", "subagent", "web_search", "fetch_content", "mcp", "lsp_diagnostics", "ast_search", "cron"]) {
		...
		if (toolUrl) assert(stderr.includes('"first_tool_completed"'), stderr);
		console.log(toolUrl ? "first-turn fetch: real local HTTP extraction completed" : "stalled optional implementations: first request retains tools and instructions");
	} finally {
		clearTimeout(timer);
		rmSync(agentDir, { recursive: true, force: true });
	}
...
const server = createServer((_request, response) => {
	...
	response.end(`<html><head><title>Startup fixture</title></head><body><article><h1>Startup fixture</h1>${"<p>This local article verifies that the first fetch tool loads its extraction implementation and reads the complete page without an external provider.</p>".repeat(30)}</article></body></html>`);
});
