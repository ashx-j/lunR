...
function terminalStringSequenceEnd(str: string, start: number): number {
	for (let i = start; i < str.length; i++) {
		if (str[i] === "\x07" || str[i] === "\x9c") return i + 1;
		if (str[i] === "\x1b" && str[i + 1] === "\\") return i + 2;
	}
	return str.length;
}

function terminalCsiEnd(str: string, start: number): number {
	for (let i = start; i < str.length; i++) {
		const code = str.charCodeAt(i);
...
/** Remove terminal controls from untrusted text while preserving logical newlines and tabs. */
export function sanitizeTerminalText(str: string): string {
	...
	while (i < str.length) {
		...
		if (code === 0x1b) {
			const next = str[i + 1];
			if (next === "[") i = terminalCsiEnd(str, i + 2);
			else if (next === "]" || next === "P" || next === "X" || next === "^" || next === "_") {
				i = terminalStringSequenceEnd(str, i + 2);
			} else {
		...
		if (code === 0x9b) {
			i = terminalCsiEnd(str, i + 1);
			continue;
		}
		if (code === 0x90 || code === 0x98 || code === 0x9d || code === 0x9e || code === 0x9f) {
			i = terminalStringSequenceEnd(str, i + 1);
			continue;
		}
		if (
...
/** Keep only the terminal sequences generated by TUI styling and layout. */
export function sanitizeTerminalOutput(str: string, allowImageProtocol = false): string {
	if (allowImageProtocol) return str;
	...
	while (i < str.length) {
		...
		if (str[i] === "\x1b" && str[i + 1] === "]") {
			const end = terminalStringSequenceEnd(str, i + 2);
			const sequence = str.slice(i, end);
			if (
				/^\x1b\]8;[^\x00-\x1f\x7f-\x9f]*(?:\x07|\x1b\\)$/.test(sequence) ||
				/^\x1b\]133;[ABC]\x07$/.test(sequence)
			) {
				result += sequence;
			}
			i = end;
		...
		if (str.startsWith("\x1b_pi:c\x07", i)) {
			result += "\x1b_pi:c\x07";
		...
		if (str[i] === "\x1b") {
			const next = str[i + 1];
			i =
				next === "P" || next === "X" || next === "^" || next === "_"
					? terminalStringSequenceEnd(str, i + 2)
					: i + 2;
			continue;
		}
		const code = str.charCodeAt(i);
		...
		if ((code < 0x20 && code !== 0x09 && code !== 0x0a) || code === 0x7f || (code >= 0x80 && code <= 0x9f)) {
			i++;
			continue;
		}
		const char = String.fromCodePoint(str.codePointAt(i)!);
