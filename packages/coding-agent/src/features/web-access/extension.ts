...
// lunr: resolved shortcut keys (config-driven). Read at factory-init time for
// the registrations and exposed for InteractiveMode's direct key handling.
export function getWebAccessShortcutKeys(): { curate: string; activity: string } {
	const initConfig = loadConfigForExtensionInit();
	return {
		curate: initConfig.shortcuts?.curate || DEFAULT_SHORTCUTS.curate,
		activity: initConfig.shortcuts?.activity || DEFAULT_SHORTCUTS.activity,
	};
}

// lunr: curator workflow get/set for core (/settings "Search curator" row) —
// writes through this feature's own web-search.json saveConfig path, the same
// single source of truth as /curator. Formerly the @lunr/search-curator
// globalThis bridge (deleted when the feature was absorbed into core).
export function getSearchCuratorWorkflow(): WebSearchWorkflow {
	return resolveWorkflow(loadConfigForExtensionInit().workflow, true);
}

export function setSearchCuratorWorkflow(workflow: WebSearchWorkflow): void {
	saveConfig({ workflow });
}

export default function (pi: ExtensionAPI) {
	const { curate: curateKey, activity: activityKey } = getWebAccessShortcutKeys();
	const initConfig = loadConfigForExtensionInit();
