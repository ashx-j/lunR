...
// lunr: absorbed into core — import from core modules directly instead of the
// package barrel (was `@earendil-works/pi-coding-agent`).
...
// lunr: reserved names track EVERY built-in slash command so user templates can
// never shadow core dispatch — derived from BUILTIN_SLASH_COMMANDS in
// core/slash-commands.ts (source of truth). The extras below cover the hidden
// dispatch-only commands (interactive-mode.ts) and this feature's own commands.
