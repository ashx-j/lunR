...
import { isTierModeEnabled } from "../../../../core/model-tiers.ts";
...
export function buildSubagentToolDescription(config: Pick<ExtensionConfig, "toolDescriptionMode"> = {}, options?: ToolDescriptionOptions): string {
	...
	// lunr: append model-tier guidance only when tier mode is enabled in core settings.
	if (isTierModeEnabled()) return `${description}\n\n${MODEL_TIER_GUIDANCE}`;
	return description;
}
