// @ts-nocheck
// lunr: model tiers are a core module now (the @lunr/model-tiers bridge is gone).
import { getTierModel, isTierModeEnabled } from "../../../../../core/model-tiers.ts";
...
// lunr: model tiers — resolve a subagent `tier` param through lunR core's
// model-tiers module (the @lunr/model-tiers bridge is gone). Disabled tier mode,
// unknown tier, or unconfigured tier all resolve to undefined so callers fall
...
export function resolveTierModelOverride(tier: unknown): string | undefined {
	...
	try {
		if (!isTierModeEnabled()) return undefined;
		const model = getTierModel(tierName);
