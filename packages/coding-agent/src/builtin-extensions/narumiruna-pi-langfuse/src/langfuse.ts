	...
	return function langfuse(pi: ExtensionAPI) {
		...
		pi.on("session_start", async (_event, ctx) => {
			...
			if (!result.ok) {
				...
				// Built-in extension: don't warn about missing config on every startup.
				// Only notify if the config exists but is invalid.
				if (!result.reason.startsWith("Configuration file not found:")) {
